...
public class ArrivalOfSNSUserStore {
    ...
    public int getSize (ArrayList<ArrivalOfSNSUser> knows){
        return knows.size();

    }

    public void removeUserFromWaitingRoom(String snsUserNumber){
        ArrivalOfSNSUser userToBeRemoved = null;
        for (ArrivalOfSNSUser user : knows){
            if (snsUserNumber.equals(user.getSNSUserNumber())){
                userToBeRemoved = user;
                break;
            }
        }
        knows.remove(userToBeRemoved);
    }

public void show(){
        for (ArrivalOfSNSUser a : knows){
